-- Create lottery (scratch card) tables for Shopline integration
-- Migration: 20260123000000_create_lottery_tables.sql
...
-- =============================================
CREATE POLICY "Allow all for service role" ON lottery_campaigns
    FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "Allow all for service role" ON lottery_prizes
    FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "Allow all for service role" ON lottery_participants
    FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "Allow all for service role" ON lottery_results
    FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "Allow all for service role" ON lottery_admin_logs
    FOR ALL USING (true) WITH CHECK (true);

-- =============================================
-- Comments for documentation
-- =============================================
COMMENT ON TABLE lottery_campaigns IS 'Scratch card lottery campaigns for Shopline stores';
COMMENT ON TABLE lottery_prizes IS 'Available prizes for each lottery campaign';
COMMENT ON TABLE lottery_participants IS 'Track users who participated in lottery campaigns';
COMMENT ON TABLE lottery_results IS 'Individual scratch results and redemption records';
COMMENT ON TABLE lottery_admin_logs IS 'Admin action audit logs for lottery management';

COMMENT ON COLUMN lottery_prizes.probability IS 'Probability of winning this prize (0.0000 to 1.0000)';
COMMENT ON COLUMN lottery_prizes.prize_type IS 'Type of prize: physical (實體商品), coupon (折價券), points (點數), free_shipping (免運), discount (折扣), none (未中獎)';
COMMENT ON COLUMN lottery_results.redemption_code IS 'Unique code for prize redemption verification';
COMMENT ON COLUMN lottery_participants.shopline_customer_id IS 'Customer ID from Shopline platform';
